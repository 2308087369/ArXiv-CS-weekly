# ArXiv Weekly - 2026年1月第二期

> **本周关键词**：连续化重构、形式化约束、多模态抗噪、工业级鲁棒性、边缘智能

## 目录

- [1. AI研究范式的分化与深水区探索](#1-ai研究范式的分化与深水区探索)
- [2. 语言建模的理论突破：打破离散桎梏](#2-语言建模的理论突破打破离散桎梏)
  - [2.1 Token Maturation：自回归语言模型的连续动力学](#21-token-maturation自回归语言模型的连续动力学)
  - [2.2 SemPA：基于语义偏好对齐的句向量优化](#22-sempa基于语义偏好对齐的句向量优化)
- [3. 智能体工程的安全性革命：霍尔契约与形式化验证](#3-智能体工程的安全性革命霍尔契约与形式化验证)
  - [3.1 ToolGate：基于契约的工具执行验证](#31-toolgate基于契约的工具执行验证)
  - [3.2 BackdoorAgent：LLM智能体的统一后门攻击框架](#32-backdooragentllm智能体的统一后门攻击框架)
- [4. 多模态系统的统一与抗噪：迈向工业级鲁棒性](#4-多模态系统的统一与抗噪迈向工业级鲁棒性)
  - [4.1 Qwen3-VL：统一的多模态检索与排序框架](#41-qwen3-vl统一的多模态检索与排序框架)
  - [4.2 Lost in the Noise：推理模型在噪声干扰下的失效](#42-lost-in-the-noise推理模型在噪声干扰下的失效)
  - [4.3 通用多图像编辑与统一多模态模型](#43-通用多图像编辑与统一多模态模型)
- [5. 科学计算与边缘智能：效率与物理约束](#5-科学计算与边缘智能效率与物理约束)
  - [5.1 StablePDENet：增强算子学习的稳定性](#51-stablepdenet增强算子学习的稳定性)
  - [5.2 训练规模再审视：Token数、能耗与参数效率](#52-训练规模再审视token数能耗与参数效率)
  - [5.3 FlyPose：高空视角下的鲁棒人体姿态估计](#53-flypose高空视角下的鲁棒人体姿态估计)
- [6. 总结与展望](#6-总结与展望)
...
### **2.1 Token Maturation：自回归语言模型的连续动力学**

> **Token Maturation: Autoregressive Language Generation via Continuous Token Dynamics**
>
> [![arXiv](https://img.shields.io/badge/arXiv-2601.04854-b31b1b.svg)](https://arxiv.org/abs/2601.04854) [**PDF下载**](../Resource/第二期/2601.04854v1.pdf)
...
<center>图1：立即承诺与Token成熟机制对比。(A) 标准自回归解码每一步都提交一个离散Token，导致早期决策不可逆。(B) Token成熟机制维护一个随时间演化的连续“液体尾部”Token表征，将离散化推迟到最终提交步骤。</center>
...
1. 连续表征（Continuous Representation）：  
   设词表嵌入矩阵为 $E \in \mathbb{R}^{|V| \times d}$。模型在每个位置 $t$ 预测的是一个连续向量 $z_t \in \mathbb{R}^d$，而非分类分布。  
2. 液体尾部（Liquid Tail）与动力学更新：  
   ...
   * $\alpha_i$ 是**成熟率（Maturation Rate）**或动量系数，通常设计为一个随时间衰减的函数（Alpha Profile）。对于接近“提交点”的Token，$\alpha$ 接近1，表示其通过当前预测进行大幅修正；对于较远的Token，$\alpha$ 较小，允许其保持模糊性。  
...
### **2.2 SemPA：基于语义偏好对齐的句向量优化**

> **SemPA: Improving Sentence Embeddings of LLMs through Semantic Preference Alignment**
>
> [![arXiv](https://img.shields.io/badge/arXiv-2601.05075-b31b1b.svg)](https://arxiv.org/abs/2601.05075) [**PDF下载**](../Resource/第二期/2601.05075v1.pdf)
...
<center>图2：句向量嵌入方法对比。上图：仅编码器模型的对比学习。下图：基于语义偏好对齐的LLM句向量优化。</center>
...
* 优化目标是最大化LLM生成的嵌入向量 $v_q$ 与 $v_{p^+}$ 的相似度，同时最小化与 $v_{p^-}$ 的相似度，且引入约束项以防止模型遗忘其生成能力：
...
<center>图3：SemPA 方法整体流程。(a) 使用NLI数据集构建释义生成偏好对。(b) 对LLM进行语义DPO训练。(c) 使用PromptEOL模板获取最终句向量。</center>
...
<center>图4：ToolGate 框架概览。该框架基于霍尔逻辑构建，将工具调用过程形式化为一系列受约束的逻辑推理步骤，并持续维护受信任状态以验证工具调用条件。</center>
...
<center>图5：BackdoorAgent 框架。BackdoorAgent 在智能体工作流的规划、记忆和工具阶段暴露了显式接口，并提供了支持可配置执行、攻击注入和轨迹记录的运行时环境。</center>
...
<center>图6：统一多模态表征空间示意图。Qwen3-VL-Embedding 系列模型将多源数据（文本、图像、视觉文档和视频）映射到公共流形中。</center>
...
<center>图7：Qwen3-VL-Embedding 和 Qwen3-VL-Reranker 的多阶段训练流水线。</center>
...
### **4.2 Lost in the Noise：推理模型在噪声干扰下的失效**

> **Lost in the Noise: How Reasoning Models Fail with Contextual Distractors**
>
> [![arXiv](https://img.shields.io/badge/arXiv-2601.07226-b31b1b.svg)](https://arxiv.org/abs/2601.07226) [**PDF下载**](../Resource/第二期/2601.07226v1.pdf)
...
<center>图8：纯净基准与 NoisyBench 的对比。显示模型在无菌环境下表现良好，但在真实噪声（随机文档、无关对话历史、强负干扰项）下失效，揭示了对齐、推理和RAG鲁棒性方面的弱点。</center>
...
>
> [![arXiv](https://img.shields.io/badge/arXiv-2601.05572-b31b1b.svg)](https://arxiv.org/abs/2601.05572) [**PDF下载**](../Resource/第二期/2601.05572v1.pdf)
...
<center>图9：混合UMM与本文设计中VAE后视觉Token编码的演示。视觉Token的排列缺乏分隔和图像索引意识，可能导致实例身份混淆。</center>
...
### **5.3 FlyPose：高空视角下的鲁棒人体姿态估计**

> **FlyPose: Towards Robust Human Pose Estimation From Aerial Views**
>
> [![arXiv](https://img.shields.io/badge/arXiv-2601.05747-b31b1b.svg)](https://arxiv.org/abs/2601.05747) [**PDF下载**](../Resource/第二期/2601.05747v1.pdf)
...
<center>图12：FlyPose-104 数据集中的两个示例，展示了带有手动标注边界框和姿态的图像，特别是下半身和面部关节的频繁自遮挡（红色标记）。</center>
