...
在优化理论中，损失函数曲面的 **锐度（Sharpness）** 由海森矩阵（Hessian Matrix，即二阶导数矩阵）的最大特征值 $\lambda_{\text{max}}$ 决定。

* **平坦极小值（Flat Minima）**：$\lambda_{\text{max}}$ 较小，意味着参数的微小扰动不会导致 Loss 剧烈变化。这通常与良好的**泛化能力**相关。
* **尖锐极小值（Sharp Minima）**：$\lambda_{\text{max}}$ 较大，意味着模型处于一个“悬崖”边缘，泛化能力通常较差。
